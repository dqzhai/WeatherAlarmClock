...
import xin.trpay.weather.Listener.RecordCheckChangedListener;
import xin.trpay.weather.activities.RecordDeleteActivity;
import xin.trpay.weather.adapter.RecordDeleteAdapter;
import xin.trpay.weather.bean.RecordDeleteItem;
import xin.trpay.weather.util.MyUtil;
import xin.trpay.weather.util.RecordItemDeleteComparator;
...
public class RecordDeleteBatchFragment extends BaseFragment implements
    ...
    private void initAdapter() {
        ...
        mTitleFormat = getString(xin.trpay.weather.R.string.selected_xx_item);
    }
    ...
    public View onCreateView(LayoutInflater inflater,
        ...
        View view = inflater.inflate(xin.trpay.weather.R.layout.fm_record_delete_batch,
                container, false);
        // 录音批量删除画面
        ViewGroup viewGroup = (ViewGroup) view
                .findViewById(xin.trpay.weather.R.id.record_delete_batch_llyt);
        // 设置页面背景
        MyUtil.setBackground(viewGroup, getActivity());

        mColorWhite = getResources().getColor(xin.trpay.weather.R.color.white_trans90);
        mColorWhiteTrans = getResources().getColor(xin.trpay.weather.R.color.white_trans60);

        // 录音ListView
        ListView recordLv = (ListView) view.findViewById(xin.trpay.weather.R.id.record_delete_batch_lv);
        ...
        mTitleTv = (TextView) view.findViewById(xin.trpay.weather.R.id.title_tv);
        mTitleTv.setText(String.format(mTitleFormat, 0));

        // 取消按钮
        ImageView cancelBtn = (ImageView) view.findViewById(xin.trpay.weather.R.id.cancel_btn);
        mSelectAllBtn = (TextView) view.findViewById(xin.trpay.weather.R.id.select_all_btn);
        mSelectNoneBtn = (TextView) view.findViewById(xin.trpay.weather.R.id.select_none_btn);
        mDeleteBtn = (Button) view.findViewById(xin.trpay.weather.R.id.delete_btn);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            // 取消按钮
            case xin.trpay.weather.R.id.cancel_btn:
                getActivity().finish();
                break;
            // 全选按钮
            case xin.trpay.weather.R.id.select_all_btn:
                // 隐藏全选显示取消全选按钮
                ...
                mDeleteBtn.setBackgroundResource(xin.trpay.weather.R.drawable.bg_btn_sure);
                ...
                break;
            // 取消全选按钮
            case xin.trpay.weather.R.id.select_none_btn:
                // 隐藏取消全选显示全选按钮
                ...
                mDeleteBtn.setClickable(false);
                mDeleteBtn
                        .setBackgroundResource(xin.trpay.weather.R.drawable.shape_circle_btn_sure_invalidate);
                ...
                break;
            // 删除按钮
            case xin.trpay.weather.R.id.delete_btn:
                // 弹出删除确认对话框
        ...
        if (requestCode == REQUEST_DELETE) {
            ...
            if (result) {
                ToastUtil.showShortToast(getActivity(),
                        getString(xin.trpay.weather.R.string.delete_success));
            }
    ...
    private void setRingList() {
        if (!MyUtil.isHasSDCard()) {
            ToastUtil.showShortToast(getActivity(), getString(xin.trpay.weather.R.string.no_sd_card));
            return;
        }
        // 录音文件路径

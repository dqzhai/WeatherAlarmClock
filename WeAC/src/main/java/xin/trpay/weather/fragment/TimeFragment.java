...
import xin.trpay.weather.Listener.OnVisibleListener;
import xin.trpay.weather.activities.RingSelectActivity;
import xin.trpay.weather.activities.TimerOnTimeActivity;
import xin.trpay.weather.bean.Event.TimerOnTimeEvent;
import xin.trpay.weather.bean.Event.TimerStartEvent;
import xin.trpay.weather.bean.model.TimeModel;
import xin.trpay.weather.common.WeacConstants;
import xin.trpay.weather.service.CountDownService;
import xin.trpay.weather.util.LogUtil;
import xin.trpay.weather.util.MyUtil;
import xin.trpay.weather.util.OttoAppConfig;
import xin.trpay.weather.view.MyTimer;
...
public class TimeFragment extends LazyLoadFragment implements View.OnClickListener,
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        final View view = inflater.inflate(xin.trpay.weather.R.layout.fm_time, container, false);
    ...
    private void assignViews(View view) {
        mStartLLyt = (ViewGroup) view.findViewById(xin.trpay.weather.R.id.btn_start_llyt);
        mStartLLyt2 = (ViewGroup) view.findViewById(xin.trpay.weather.R.id.btn_start_llyt2);

        mStartBtn = (TextView) view.findViewById(xin.trpay.weather.R.id.btn_start);
        mStartBtn2 = (TextView) view.findViewById(xin.trpay.weather.R.id.btn_start2);
        mStopBtn = (TextView) view.findViewById(xin.trpay.weather.R.id.btn_stop);
        // 重置按钮
        TextView resetBtn = (TextView) view.findViewById(xin.trpay.weather.R.id.btn_reset);
        mQuickBtn = (TextView) view.findViewById(xin.trpay.weather.R.id.btn_quick);
        // 铃声按钮
        TextView ringBtn = (TextView) view.findViewById(xin.trpay.weather.R.id.btn_ring);
        ...
        mTimer = (MyTimer) view.findViewById(xin.trpay.weather.R.id.timer);
...
//        mTimer.setStartTime(0, 0, 0, true, false);
    ...
    private void showTimeLayout(View view) {
        // 计时布局
        ViewStub viewStub = (ViewStub) view.findViewById(xin.trpay.weather.R.id.viewstub_time);
        viewStub.inflate();

        // 加载中进度框
        ViewGroup progressBar = (ViewGroup) view.findViewById(xin.trpay.weather.R.id.progress_bar_llyt);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            // 开始
            case xin.trpay.weather.R.id.btn_start:
                ...
                break;
            // 快捷便签
            case xin.trpay.weather.R.id.btn_quick:
                displayQuickOptions();
                break;
            // 开始2
            case xin.trpay.weather.R.id.btn_start2:
                startCountDown();
                setStopVisible();
                break;
            // 暂停
            case xin.trpay.weather.R.id.btn_stop:
                stopAlarmClockTimer();
                mTimer.stop();
                stopCountDown();
                setStart2Visible();
                break;
            // 重置
            case xin.trpay.weather.R.id.btn_reset:
                stopAlarmClockTimer();
                mTimer.reset();
                stopCountDown();
                setStratLlytVisible();
                break;
            // 铃声选择
            case xin.trpay.weather.R.id.btn_ring:
                processRingSelect();
                break;
            // 午睡
            case xin.trpay.weather.R.id.btn_siesta:
                processQuickTimer(0, 45, 0);
                break;
            // 面膜
            case xin.trpay.weather.R.id.btn_facial_mask:
                processQuickTimer(0, 15, 0);
                break;
            // 泡面
            case xin.trpay.weather.R.id.btn_instant_noodles:
                processQuickTimer(0, 3, 0);
                break;
            // 跑步
            case xin.trpay.weather.R.id.btn_run:
    ...
    private void processRingSelect() {
        ...
        String ringName = shares.getString(WeacConstants.RING_NAME_TIMER, getString(xin.trpay.weather.R.string.default_ring));
    ...
    private void displayQuickOptions() {
        if (!isQuickTimerOptionsInitialized) {
            ...
            View convertView = LayoutInflater.from(getActivity()).inflate(xin.trpay.weather.R.layout.ppv_timer, null);
            ...
            mPopupWindow.setBackgroundDrawable(new ColorDrawable(getResources().getColor(xin.trpay.weather.R.color.black_trans50)));
            ...
            // 午睡
            TextView siestaBtn = (TextView) convertView.findViewById(xin.trpay.weather.R.id.btn_siesta);
            // 面膜
            TextView facialMask = (TextView) convertView.findViewById(xin.trpay.weather.R.id.btn_facial_mask);
            // 泡面
            TextView instantNoodles = (TextView) convertView.findViewById(xin.trpay.weather.R.id.btn_instant_noodles);
            // 跑步
            TextView run = (TextView) convertView.findViewById(xin.trpay.weather.R.id.btn_run);

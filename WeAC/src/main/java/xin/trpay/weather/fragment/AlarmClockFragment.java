...
import xin.trpay.weather.Listener.OnItemClickListener;
import xin.trpay.weather.activities.AlarmClockEditActivity;
import xin.trpay.weather.activities.AlarmClockNewActivity;
import xin.trpay.weather.activities.ShakeExplainActivity;
import xin.trpay.weather.adapter.AlarmClockAdapter;
import xin.trpay.weather.bean.AlarmClock;
import xin.trpay.weather.bean.Event.AlarmClockDeleteEvent;
import xin.trpay.weather.bean.Event.AlarmClockUpdateEvent;
import xin.trpay.weather.bean.Event.ShakeExplainCloseEvent;
import xin.trpay.weather.common.WeacConstants;
import xin.trpay.weather.db.AlarmClockOperate;
import xin.trpay.weather.util.MyUtil;
import xin.trpay.weather.util.OttoAppConfig;
import xin.trpay.weather.util.ToastUtil;
import xin.trpay.weather.view.ErrorCatchLinearLayoutManager;
...
//        getLoaderManager().initLoader(1, null, this);
    ...
    public View onCreateView(final LayoutInflater inflater,
        ...
        View view = inflater.inflate(xin.trpay.weather.R.layout.fm_alarm_clock, container, false);

        mEmptyView = (LinearLayout) view
                .findViewById(xin.trpay.weather.R.id.alarm_clock_empty);

        mRecyclerView = (RecyclerView) view.findViewById(xin.trpay.weather.R.id.list_view);
...
//        OverScrollDecoratorHelper.setUpOverScroll(mRecyclerView, OverScrollDecoratorHelper.ORIENTATION_VERTICAL);
        ...
        // 操作栏新建按钮
        ImageView newAction = (ImageView) view.findViewById(xin.trpay.weather.R.id.action_new);
        newAction.setOnClickListener(this);

        // 编辑闹钟
        mEditAction = (ImageView) view.findViewById(xin.trpay.weather.R.id.action_edit);
        mEditAction.setOnClickListener(this);

        // 完成按钮
        mAcceptAction = (ImageView) view.findViewById(xin.trpay.weather.R.id.action_accept);
    ...
    class OnItemClickListenerImpl implements OnItemClickListener {
        ...
        public void onItemClick(View view, int position) {
            ...
            // 启动移动进入效果动画
            getActivity().overridePendingTransition(xin.trpay.weather.R.anim.move_in_bottom,
                    0);
        }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case xin.trpay.weather.R.id.action_new:
                // 不响应重复点击
                if (MyUtil.isFastDoubleClick()) {
                    return;
                }
                ...
                // 启动渐变放大效果动画
                getActivity().overridePendingTransition(xin.trpay.weather.R.anim.zoomin, 0);
                break;
                ...
                public void onSensorChanged(SensorEvent event) {
                    ...
                    if (xValue > 15 || yValue > 15 || zValue > 15) {
                        if (mDeletedAlarmClock != null) {
                            ...
                            ToastUtil.showLongToast(getActivity(), getString(xin.trpay.weather.R.string.retrieve_alarm_clock_success));
                        }
                    }
                }
    ...
    private void showAlarmExplain() {
        if (isShow()) {
            new AlertDialogWrapper.Builder(getActivity())
                    .setTitle(getActivity().getString(xin.trpay.weather.R.string.warm_tips_title))
                    .setMessage(getActivity().getString(xin.trpay.weather.R.string.warm_tips_detail))
                    .setPositiveButton(xin.trpay.weather.R.string.roger, new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {

                        }
                    })
                    .setNegativeButton(xin.trpay.weather.R.string.no_tip, new DialogInterface.OnClickListener() {
                        @Override
                        public void onClick(DialogInterface dialog, int which) {

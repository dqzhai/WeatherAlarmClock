...
public class LocalAlbumAdapter extends ArrayAdapter<ImageBucket> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = LayoutInflater.from(mContext).inflate(
                    xin.trpay.weather.R.layout.lv_local_album, parent, false);
        ...
        if (null != imageBucket && imageBucket.bucketList.size() > 0) {
            ...
            if (!TextUtils.isEmpty(imagePath)) {
                Glide.with(mContext).load("file://" + imagePath).
                        placeholder(xin.trpay.weather.R.color.default_image_background).into(viewHolder.mAlbumThumbnailIv);
            }
            ...
            if (!TextUtils.isEmpty(title)) {
                viewHolder.mAlbumTitleTv.setText(String.format(
                        mContext.getString(xin.trpay.weather.R.string.picture_name_number), title, count));
            }
        }
        return convertView;
    }
    ...
    public class ViewHolder {
        ...
        public ViewHolder(View root) {
            mAlbumThumbnailIv = (ImageView) root.findViewById(xin.trpay.weather.R.id.album_thumbnail_iv);
            mAlbumTitleTv = (TextView) root.findViewById(xin.trpay.weather.R.id.album_title_tv);
        }
    }
}

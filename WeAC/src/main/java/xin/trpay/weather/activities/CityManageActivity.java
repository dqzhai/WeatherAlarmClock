...
import xin.trpay.weather.Listener.DBObserverListener;
import xin.trpay.weather.Listener.HttpCallbackListener;
import xin.trpay.weather.Listener.NotifyListener;
import xin.trpay.weather.adapter.CityManageAdapter;
import xin.trpay.weather.bean.CityManage;
import xin.trpay.weather.bean.WeatherDaysForecast;
import xin.trpay.weather.bean.WeatherInfo;
import xin.trpay.weather.common.WeacConstants;
import xin.trpay.weather.db.WeatherDBOperate;
import xin.trpay.weather.util.HttpUtil;
import xin.trpay.weather.util.LogUtil;
import xin.trpay.weather.util.MyUtil;
import xin.trpay.weather.util.ToastUtil;
import xin.trpay.weather.util.WeatherUtil;
...
public class CityManageActivity extends BaseActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(xin.trpay.weather.R.layout.activity_city_manage);
    ...
    private void initViews() {
        ...
        mGridView = (GridView) findViewById(xin.trpay.weather.R.id.gv_city_manage);
        ...
        // 编辑闹钟
        mEditBtn = (ImageView) findViewById(xin.trpay.weather.R.id.action_edit);
        mEditBtn.setOnClickListener(this);
        // 完成按钮
        mAcceptBtn = (ImageView) findViewById(xin.trpay.weather.R.id.action_accept);
        mAcceptBtn.setOnClickListener(this);
        // 刷新按钮
        mRefreshBtn = (ImageView) findViewById(xin.trpay.weather.R.id.action_refresh);
        mRefreshBtn.setOnClickListener(this);
        // 取消刷新按钮
        mCancelRefreshBtn = (ImageView) findViewById(xin.trpay.weather.R.id.action_refresh_cancel);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case xin.trpay.weather.R.id.action_return:
                ...
                break;
            // 编辑按钮
            case xin.trpay.weather.R.id.action_edit:
            ...
            case xin.trpay.weather.R.id.action_accept:
                ...
                break;
            // 刷新按钮
            case xin.trpay.weather.R.id.action_refresh:
                // 列表为空（只有添加按钮）不刷新
                ...
                break;
            // 取消更新
            case xin.trpay.weather.R.id.action_refresh_cancel:
                mIsRefreshing = false;
    ...
    private void modifyCity() {
        ...
        myFinish(share.getString(WeacConstants.DEFAULT_CITY_NAME, null),
                share.getString(WeacConstants.DEFAULT_WEATHER_CODE, getString(xin.trpay.weather.R.string.auto_location)));
    }
                    ...
                    public void onFinish(String response) {
                        try {
                            if (!response.contains("error")) {
                                ...
                                switch (position) {
                                    case -1:
                                        runOnUi(getString(xin.trpay.weather.R.string.no_data), position);
//                                        runOnUi(getString(R.string.no_city_info), position);
                                        break;
                                    case -2:
                                        runOnUi(getString(xin.trpay.weather.R.string.can_not_find_current_location), position);
                                        break;
                                    default:
                                        runOnUi(getString(xin.trpay.weather.R.string.no_data), -1);
                                        break;
                                }
                            }
...
//                            runOnUi(getString(R.string.unknown_error), position);
                    ...
                    public void onError(Exception e) {
                        try {
                            ...
                            // 添加城市
                            if (position == -1) {
                                runOnUi(getString(xin.trpay.weather.R.string.add_city_fail), position);
                                // 添加定位
                            } else if (position == -2) {
                                runOnUi(getString(xin.trpay.weather.R.string.add_location_fail), position);
                            } else {
                                if (address != null) {
                                    runOnUi(String.format(getString(xin.trpay.weather.R.string.refresh_fail), cityName), position);
                                    // 自动定位
                                } else {
                                    runOnUi(String.format(getString(xin.trpay.weather.R.string.refresh_fail), getString(
                                            xin.trpay.weather.R.string.auto_location)), position);
                                }
                            }
                        } catch (Exception e1) {
//                            runOnUi(getString(R.string.unknown_error), position);
    ...
    class SetCityInfoRunnable implements Runnable {
        ...
        public void run() {
            ...
            if (mPosition == -1) {
                ...
                processCityManage(getString(xin.trpay.weather.R.string.auto_location), mWeatherInfo.getCity(),
                        getString(xin.trpay.weather.R.string.auto_location));
            } else {
                // 取得城市管理列表对应的城市管理item
    ...
    private void refresh(int position) {
        ...
        if (cityManage1.getLocationCity() == null) {
            ...
            address = getString(xin.trpay.weather.R.string.address_weather, cityManage1.getWeatherCode());
        } else {
    ...
    private void setCityManageInfo(CityManage cityManage, WeatherInfo weatherInfo) {
        if (weatherInfo.getWeatherDaysForecast().size() >= 5) {
            ...
            cityManage.setTempHigh(getString(xin.trpay.weather.R.string.dash));
            cityManage.setTempLow(getString(xin.trpay.weather.R.string.dash));
            cityManage.setWeatherType(getString(xin.trpay.weather.R.string.no));
            cityManage.setWeatherTypeDay(getString(xin.trpay.weather.R.string.no));
            cityManage.setWeatherTypeNight(getString(xin.trpay.weather.R.string.no));
        }
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_CITY_MANAGE) {
            ...
            if (weatherCode != null) {
                ...
                queryFormServer(getString(xin.trpay.weather.R.string.address_weather, weatherCode),
                        ADD_CITY, null);
                // 添加定位
            } else {

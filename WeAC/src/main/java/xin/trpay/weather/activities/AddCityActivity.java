...
import xin.trpay.weather.adapter.AddCityAdapter;
import xin.trpay.weather.bean.County;
import xin.trpay.weather.common.WeacConstants;
import xin.trpay.weather.db.WeatherDBOperate;
import xin.trpay.weather.util.LogUtil;
import xin.trpay.weather.util.MyUtil;
import xin.trpay.weather.util.ToastUtil;
...
public class AddCityActivity extends BaseActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(xin.trpay.weather.R.layout.activity_add_city);
        LinearLayout backGround = (LinearLayout) findViewById(xin.trpay.weather.R.id.city_manage_background);
        // 设置页面高斯模糊背景
    ...
    private void initViews() {
        // 返回按钮
        ImageView returnBtn = (ImageView) findViewById(xin.trpay.weather.R.id.action_return);
        returnBtn.setOnClickListener(this);

        // 更多城市和返回按钮
        LinearLayout moreCityAndReturnBtn = (LinearLayout) findViewById(xin.trpay.weather.R.id.more_city_and_return_btn);
        moreCityAndReturnBtn.setOnClickListener(this);
        mMoreCityAndReturnBtnTv = (TextView) findViewById(xin.trpay.weather.R.id.more_city_and_return_btn_tv);

        // 编辑框
        mSearchCityEtv = (EditText) findViewById(xin.trpay.weather.R.id.search_city_edit_tv);
        ...
        // 城市列表GridView
        mGvTitle = (TextView) findViewById(xin.trpay.weather.R.id.gv_add_city_title);
        GridView addCityGridView = (GridView) findViewById(xin.trpay.weather.R.id.gv_add_city);
        addCityGridView.setAdapter(mAddAddCityAdapter);
        addCityGridView.setOnItemClickListener(new AddCityOnItemClickListener());

        // 清除按钮
        mClearBtn = (ImageView) findViewById(xin.trpay.weather.R.id.clear_btn);
        mClearBtn.setOnClickListener(this);

        mWeatherCodes = getResources().getStringArray(xin.trpay.weather.R.array.city_china_weather_code);
        mCountries = getResources().getStringArray(xin.trpay.weather.R.array.city_china);
        mCountriesPinyin = getResources().getStringArray(xin.trpay.weather.R.array.city_china_pinyin);
        mCountriesEn = getResources().getStringArray(xin.trpay.weather.R.array.city_china_en);

        // 热门城市视图
        mHotCityLlyt = (LinearLayout) findViewById(xin.trpay.weather.R.id.city_contents);
        // 无匹配提示
        mNoMatchedCityTv = (TextView) findViewById(xin.trpay.weather.R.id.no_matched_city_tv);
        // 查找城市
        mSearchCityLv = (ListView) findViewById(xin.trpay.weather.R.id.lv_search_city);
        mSearchCityList = new ArrayList<>();
        mSearchCityAdapter = new ArrayAdapter<>(
                this, xin.trpay.weather.R.layout.lv_search_city, mSearchCityList);
                ...
                for (int i = 0; i < length; i++) {
                    ...
                    if (mCountries[i].contains(cityName) ||
                        ...
                        ForegroundColorSpan span = new ForegroundColorSpan(getResources().
                                getColor(xin.trpay.weather.R.color.white_trans90));
            ...
            switch (mCurrentLevel) {
                case LEVEL_HOT_CITY:
                    if (!MyUtil.isNetworkAvailable(AddCityActivity.this)) {
                        ToastUtil.showShortToast(AddCityActivity.this, getString(xin.trpay.weather.R.string.internet_error));
                        return;
                    }
                    ...
                    if (isCityNoAdd(cityName)) {
                        ...
                        ToastUtil.showShortToast(AddCityActivity.this, getString(
                                xin.trpay.weather.R.string.city_already_added, cityName));
                    }
                    break;
                // 省
                case LEVEL_PROVINCE:
                ...
                case LEVEL_COUNTY:
                    ...
                    if (isCityNoAdd(selectedCounty.getCountyName())) {
                        ...
                        ToastUtil.showShortToast(AddCityActivity.this, getString(
                                xin.trpay.weather.R.string.city_already_added, selectedCounty.getCountyName()));
                    }
                    break;
            }
        }
    ...
    class SearchCityOnItemClickListener implements AdapterView.OnItemClickListener {
        ...
        public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
            ...
            if (isCityNoAdd(city)) {
                ...
                ToastUtil.showShortToast(AddCityActivity.this, getString(
                        xin.trpay.weather.R.string.city_already_added, city));
            }
        }
    }
    ...
    private void startLocation() {
        if (!MyUtil.isNetworkAvailable(this)) {
            ToastUtil.showShortToast(this, getString(xin.trpay.weather.R.string.internet_error));
            return;
        }

        int number = WeatherDBOperate.getInstance().queryCityManage(getString(xin.trpay.weather.R.string.auto_location));
        // 当已添加自动定位
        if (number == 1) {
            ToastUtil.showShortToast(AddCityActivity.this, getString(xin.trpay.weather.R.string.location_already_added));
            return;
        }
        ...
        showProgressDialog(getString(xin.trpay.weather.R.string.now_locating));
...
//        option.setEnableSimulateGps(false);//可选，默认false，设置是否需要过滤gps仿真结果，默认需要
    ...
    class MyLocationListener implements BDLocationListener {
        ...
        public void onReceiveLocation(BDLocation location) {
            ...
            if (location == null) {
                ToastUtil.showShortToast(AddCityActivity.this, getString(xin.trpay.weather.R.string.location_fail));
                return;
            }
            ...
            if (161 == location.getLocType() && address != null) {
                ...
                if (cityName != null) {
                    ...
                    ToastUtil.showLongToast(AddCityActivity.this, getString(xin.trpay.weather.R.string.can_not_find_current_location));
                }
                // 定位失败
            } else {
                LogUtil.d(LOG_TAG, "error code: " + location.getLocType());
                Intent intent = new Intent(AddCityActivity.this, MyDialogActivity.class);
                intent.putExtra(WeacConstants.TITLE, getString(xin.trpay.weather.R.string.prompt));
                intent.putExtra(WeacConstants.DETAIL, getString(xin.trpay.weather.R.string.location_fail));
                intent.putExtra(WeacConstants.SURE_TEXT, getString(xin.trpay.weather.R.string.retry));
    ...
    private void queryHotCities() {
        mAddCityList.clear();
        String[] city = getResources().getStringArray(xin.trpay.weather.R.array.city_hot);
        Collections.addAll(mAddCityList, city);
        mAddAddCityAdapter.notifyDataSetChanged();
        mGvTitle.setText(xin.trpay.weather.R.string.hot_city);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            // 返回按钮
            case xin.trpay.weather.R.id.action_return:
                finish();
                break;
            // 更多城市和返回按钮
            case xin.trpay.weather.R.id.more_city_and_return_btn:
                dispatchBackAction(0);
                break;
            // 清除按钮
            case xin.trpay.weather.R.id.clear_btn:
    ...
    private void dispatchBackAction(int type) {
        ...
        switch (mCurrentLevel) {
            ...
            case LEVEL_HOT_CITY:
                switch (type) {
                    case 0:
                        ...
                        queryProvinces();
                        // 设置为返回按钮
                        mMoreCityAndReturnBtnTv.setText(getString(xin.trpay.weather.R.string.back));
                        break;
                    case 1:
                        finish();
                        break;
            ...
            case LEVEL_PROVINCE:
                ...
                queryHotCities();
                // 设置为更多城市按钮
                mMoreCityAndReturnBtnTv.setText(getString(xin.trpay.weather.R.string.more_city));
                break;
            // 市
            case LEVEL_CITY:
                // 查询省
    ...
    private void queryProvinces() {
        mAddCityList.clear();
        XmlPullParser parser = getResources().getXml(xin.trpay.weather.R.xml.city_china);
        try {
            ...
            mAddAddCityAdapter.notifyDataSetChanged();
            mGvTitle.setText(xin.trpay.weather.R.string.china);
    ...
    private void showProgressDialog(String message) {
        if (mProgressDialog == null) {
            mProgressDialog = new Dialog(this, xin.trpay.weather.R.style.Theme_MyDialog);
            mProgressDialog.setContentView(xin.trpay.weather.R.layout.dialog_loading);
            mProgressDialog.setCancelable(false);
            TextView msg = (TextView) mProgressDialog.findViewById(xin.trpay.weather.R.id.dialog_msg);
            msg.setText(message);
